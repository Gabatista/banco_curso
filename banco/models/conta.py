...
class Conta:
    ...
    @property
    def _calcula_saldo_total(self:object) -> None:
    ...
    def depositar(self:object,valor:int) -> None:
        if valor > 0:
            ...
            print(f'Erro ao depositar, tente novamente')
    ...
    def sacar(self:object,valor:int) -> None:
        if valor > 0 and self.saldo_total >= valor:
            ...
            print(f'Saque não realizado. Tente novamente')

    def transferir(self:object,destino:object, valor:int) -> None:
        if valor > 0 and self.saldo_total >= valor:
            if self.saldo >= valor:
                self.saldo = self.saldo - valor
                self.saldo_total = self._calcula_saldo_total
            ...
            print('Transferência não realidazada')